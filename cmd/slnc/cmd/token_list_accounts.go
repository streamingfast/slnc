...
var tokenListAccountsCmd = &cobra.Command{
	Use:   "accounts {owner_addr}",
	Short: "Lists token accounts for a given user",
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		rpcCli := getClient()
		toCSV := viper.GetBool("token-list-accounts-cmd-to-csv")
		...
		accounts, err := token.FetchAccountsForOwner(rpcCli, ownerAddr)
		if err != nil {
...
func init() {
	tokenListAccountsCmd.Flags().Bool("to-csv", false, "outputs the data in csv format")
	tokenListCmd.AddCommand(tokenListAccountsCmd)
}
